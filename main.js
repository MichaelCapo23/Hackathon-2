...
app.get("/search", function(req, res) {
  ...
  requestpromise(searchOptions).then(function(dat) {
    ...
    var promises = playlists.map(function(playlist) {
      ...
      return requestpromise.get(tracksOption, function(error, response,body) {
    ...
    bluebird.all(promises).spread(function (...args) {
      console.log(args.items);
    })
    // q.all(promises).then(console.log(tracks))
    console.log('hello');

    // for (var index = 0; index < playlists.length; index++) {
    //   let tracksOptions = {
    //     method: "GET",
    //     url: `https://api.spotify.com/v1/playlists/${playlists[index]}/tracks`,
    //     headers: {
    //       Authorization: "Bearer " + access_token
    //     },
    //     json: true
